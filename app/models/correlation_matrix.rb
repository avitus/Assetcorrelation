...
class Correlation_matrix
  ...
  # ----------------------------------------------------------------------------------------------------------  
  def add_many_stocks(tickers, days)
    ...
    yahoo_quotes, shortest_ticker, error_code  = yahoo_request(tickers, days)
    if (error_code==1) # too little history to add asset
      return shortest_ticker
    end

    # 2) Convert to daily returns (!! Do we need num_companies?? or days for that matter)
    ...
    for company in 0...stock_sequences.size
      logger.debug("Stock_sequence length: #{stock_sequences[company].length}")
  ...
  def add_stock(name, new_series)
    ...
    # logger.debug("***** Std deviation is    : #{@stock_names.last}")
    # logger.debug("***** Std deviation is    : #{@stock_stddevs.last}")

    if @stock_histories.size != 1
      for col in 0...(@stock_histories.size-1)
        # logger.debug("***** Col update         : #{col}")
        @correlation_matrix[(@stock_histories.size-2)][col] = correlation(@stock_histories[col], new_series)
        # copy value into transposed position -- we should only do this if necessary
        # @correlation_matrix[col][(@stock_histories.size-2)] = @correlation_matrix[(@stock_histories.size-2)][col]
      end
    end
  ...
  def yahoo_request(tickers, period_req)    # period = number of days of history (including non-trading days)  
    ...
    if (@trading_days>0) # that is, we're adding stocks to an existing portfolio
      ...
      if (history_lengths.min >= @trading_days)
        logger.info("==== Truncating because stock being added has less history than existing portfolio")
      ...
      logger.debug("** Setting number of trading days to #{@trading_days}")
    end
    ...
    sd          = ParseDate::parsedate(@start_date)
    startdate   = Date.new(sd[0], sd[1], sd[2])

    ed          = ParseDate::parsedate(@end_date)
    enddate     = Date.new(ed[0], ed[1], ed[2])    
  ...
  def ping_yahoo(tickers, period_req)
    ...
    for i in 0...companies
      h = YahooFinance::get_historical_quotes_days(tickers[i].upcase, period_req)
      historical_quotes << h
      history_lengths   << h.size
      # This next line returns a nil error -- common problem on update_all
      # logger.debug("** Requested #{period_req} data for #{tickers[i].upcase}. Returned #{h.size} Period: #{h.last[0]} to #{h.first[0]}" )      
      # logger.debug("         Open: #{h.last[6]}   Close: #{h.first[6]}")
    end
       
    # Keep requesting historical quotes until Yahoo coughs up all the data
    # This should only require multiple requests infrequently (9/15/08 - reduced max reqs from 30 to 2)
    # Reducing number of extra requests to 1 -- this doesn't seem to make a difference very often
    # In fact, we may not even need it any more
    while ((history_lengths.sort.first != history_lengths.sort.last) && (requests <= 1))
    # logger.debug("** Mismatch in history length - this is additional request #{requests}")
      for i in 0...companies
        if (history_lengths[i] < history_lengths.sort.last)
          historical_quotes[i] = YahooFinance::get_historical_quotes_days(tickers[i].upcase, period_req)
          # logger.debug("** Requested #{period_req} data for #{tickers[i].upcase}. Returned #{historical_quotes[i].size}")          
    ...
    # logger.debug("== Shortest history: #{history_lengths.sort.first}")
    # logger.debug("== Longest history : #{history_lengths.sort.last}") 
    ...
    if history_lengths.sort.first != history_lengths.sort.last and start_end_ok(historical_quotes)
      logger.info("=== Checking for missing quote data")
      quote_insertion(historical_quotes, history_lengths.max)
    end
    # @trading_days = history_lengths.min
  ...
  def price_to_returns(yahoo_history, num_companies)   
    ...
    for company in 0...num_companies 
      ...
      logger.debug("Open: #{series_build.last} Close: #{series_build.first} Years: #{num_years}")
      logger.debug("Daily Return array length: #{series_build.length}")
      ...
      if @trading_days > 20000 # 50 years  
        ...
        logger.debug("Weekly Return array length: #{series_build.length}")
      end
  ...
  def portfolio_stdev
    ...
    #logger.info("***** Number of assets #{num_assets}")
    
    for col in 0...num_assets
      for row in col...num_assets
        #logger.info("********* Col: #{col+1} Row: #{row+1} CorCoeff: #{@correlation_matrix[row][col]} StdevRow: #{@stock_stddevs[row+1]} StdevCol: #{@stock_stddevs[col]}") 
        port_stdev += (@correlation_matrix[row][col] * @stock_stddevs[row+1] * @stock_stddevs[col] * 2)  
        #logger.info("******************* Cross: #{port_stdev} Added #{@correlation_matrix[row][col] * @stock_stddevs[row+1] * @stock_stddevs[col] * 2}")                        
      end
      port_stdev += @stock_stddevs[col] * @stock_stddevs[col]
      #logger.info("******************* Autos: #{port_stdev} Added #{@stock_stddevs[col] * @stock_stddevs[col]}") 
    end
    ...
    if port_stdev<0
      ...
      #logger.info("*!*!*!* Taking square root of : #{port_stdev} divided by #{num_assets+1} squared")
  ...
  def quote_insertion(quote_array, required_trading_days )
    ...
    logger.info("== Setting all histories to length: #{required_trading_days}")
    # For each company - check whether there are any dates missing from golden date array and add them
    logger.info("== Compiling date array")
